...
from .caire_adapter import Adapter, MixAdapter
...
class MixAdapter(nn.Module):
    def __init__(self, config, bottleneck_size=200, adapter_num=2):
        super(MixAdapter, self).__init__()
        # 2 adapters with task_id 0--1, when task_id==-1 means dont use adapter
        # one adapter for each task type
...
class GPT2AdapterModel(GPT2PreTrainedModel):
    r"""
    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
        **last_hidden_state**: ``torch.FloatTensor`` of shape ``(batch_size, sequence_length, hidden_size)``
            Sequence of hidden-states at the last layer of the model.
    ...
    """
    def __init__(self, config):
        super(GPT2AdapterModel, self).__init__(config)
        self.output_hidden_states = config.output_hidden_states
        self.output_attentions = config.output_attentions
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, past=None, head_mask=None, task_id=0):
        if past is None:
            past_length = 0
            past = [None] * len(self.h)
        else:
        ...
        all_attentions = []
        all_hidden_states = ()
        for i, (block, layer_past, adapter) in enumerate(zip(self.h, past, self.adapter_blocks)):
            if self.output_hidden_states:
                all_hidden_states = all_hidden_states + (hidden_states.view(*output_shape),)
...
class GPT2DoubleHeadsModel(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, mc_token_ids=None, lm_labels=None, mc_labels=None, token_type_ids=None,
        ...
        transformer_outputs = self.transformer(input_ids, position_ids=position_ids,
...
class GPT2DoubleHeadsAdapterModel(GPT2PreTrainedModel):
    r"""    Inputs:
        **input_ids**: ``torch.LongTensor`` of shape ``(batch_size, num_choices, sequence_length)``:
            Indices of input sequence tokens in the vocabulary.
            The second dimension of the input (`num_choices`) indicates the number of choices to score.
    ...
    def __init__(self, config):
        super(GPT2DoubleHeadsAdapterModel, self).__init__(config)
    ...
    def forward(self, input_ids, mc_token_ids=None, lm_labels=None, mc_labels=None, token_type_ids=None,
                position_ids=None, past=None, head_mask=None, task_id=None):
        # print('tom debugs')
        # print(task_id)
        transformer_outputs = self.transformer(input_ids, position_ids=position_ids,
                                               token_type_ids=token_type_ids,
                                               past=past, head_mask=head_mask, task_id=task_id[0][0])
